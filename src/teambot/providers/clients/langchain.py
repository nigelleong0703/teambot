...
class LangChainProviderClient:
    ...
    def invoke(
        ...
        *,
        ...
        normalized_stream = (
            normalize_chat_response(combined_chunk)
        ...
        if stream_error is not None or (
            ...
            if normalized.text and not text_parts:
                on_token(normalized.text)
            return normalized

        if normalized_stream.text:
            text = normalized_stream.text
        else:
            text = "".join(text_parts)
        if normalized_stream.finish_reason:
            finish_reason = normalized_stream.finish_reason
        if normalized_stream.usage:
            usage.update(normalized_stream.usage)

        return NormalizedResponse(
            text=text,
            tool_calls=list(normalized_stream.tool_calls),
